"""Tool registry for the M4 agent.

Every tool executes against the same SQLite repositories used by sync/index/
summary, so rule mode and LLM mode share one traceable evidence layer.
"""
...
@dataclass
class ToolResult:
    """Normalized tool output with a citation-friendly items list."""
...
class ToolRegistry:
    """Resolve tool names against fresh repository instances."""
